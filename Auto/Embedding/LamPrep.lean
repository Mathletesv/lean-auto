...
theorem LamGenModify.andLeft? : LamGenModify R? lval LamTerm.andLeft? true := by
  intro t₁ t₂ heq lctx hwf; dsimp
  match t₁, heq with
  | .app _ (.app _ (.base .and) _) _, Eq.refl _ =>
    cases hwf.getFn.getFn.getBase.getPcst
    apply LamValid.and_left _ hwf.getFn.getArg hwf.getArg
...
theorem LamGenModify.andRight? : LamGenModify R? lval LamTerm.andRight? true := by
  intro t₁ t₂ heq lctx hwf; dsimp
  match t₁, heq with
  | .app _ (.app _ (.base .and) _) _, Eq.refl _ =>
    cases hwf.getFn.getFn.getBase.getPcst
    apply LamValid.and_right _ hwf.getFn.getArg hwf.getArg
...
theorem LamEquiv.prop_ne_equiv_eq_not
  ...
  LamEquiv R? lval lctx (.base .prop) (.mkNot (.mkEq (.base .prop) lhs rhs)) (.mkEq (.base .prop) lhs (.mkNot rhs)) := by
...
theorem LamTerm.maxEVarSucc_prop_ne_equiv_eq_not?
  ...
  | .app _ (.base .not) (.app _ (.app _ (.base (.eq (.base .prop))) lhs) rhs), Eq.refl _ => by
    dsimp [mkEq, mkNot, maxEVarSucc]; simp [Nat.max]
...
theorem LamEquiv.prop_ne_equiv_eq_not?
  ...
  | .app _ (.base .not) (.app _ (.app _ (.base (.eq (.base .prop))) lhs) rhs), Eq.refl _ => by
    ...
    | .ofApp _ (.ofBase .ofNot) (.ofApp _ (.ofApp _ (.ofBase (.ofEq _)) Hlhs) Hrhs) =>
      apply LamEquiv.prop_ne_equiv_eq_not _ Hlhs Hrhs

theorem LamGenConv.prop_ne_equiv_eq_not? : LamGenConv R? lval LamTerm.prop_ne_equiv_eq_not? := by
  intros t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.prop_ne_equiv_eq_not? _ wf heq
...
theorem LamTerm.maxEVarSucc_equalize : (LamTerm.equalize t).maxEVarSucc = t.maxEVarSucc := by
  simp [equalize, mkEq, maxEVarSucc, Nat.max]

theorem LamEquiv.equalize (wft : LamWF lval.toLamTyVal ⟨lctx, t, .base .prop⟩) :
  LamEquiv R? lval lctx (.base .prop) t t.equalize := by
...
theorem LamEquiv.equalize?
  ...
  s = .base .prop ∧ LamEquiv R? lval lctx (.base .prop) t t' := by
  match s, heq with
  | .base .prop, Eq.refl _ =>
    apply And.intro rfl; apply LamEquiv.equalize _ wft

theorem LamGenConv.equalize? : LamGenConvWith R? lval LamTerm.equalize? := by
  intros rty t₁ t₂ heq lctx wf
  match rty, heq with
  | .base .prop, Eq.refl _ => apply LamEquiv.equalize _ wf
...
theorem LamGenConv.true_eq_false_equiv_false? : LamGenConv R? lval LamTerm.true_eq_false_equiv_false? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.true_eq_false_equiv_false? _ wf heq
...
theorem LamGenConv.false_eq_true_equiv_false? : LamGenConv R? lval LamTerm.false_eq_true_equiv_false? := by
  intros t₁ t₂ heq rty lctx wf
  have ⟨heq, hequiv⟩ := LamEquiv.false_eq_true_equiv_false? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.eq_true_equiv?
  (wft : LamWF lval.toLamTyVal ⟨lctx, t, s⟩)
  (heq : LamTerm.eq_true_equiv? t = .some t') :
  s = .base .prop ∧ LamEquiv R? lval lctx (.base .prop) t t' :=
  match t, heq with
  | .app _ (.app _ (.base (.eq _)) lhs) (.base .trueE), Eq.refl _ => by
    cases wft.getFn.getFn.getBase; exists rfl, wft
    match wft with
...
theorem LamGenConv.eq_true_equiv? : LamGenConv R? lval LamTerm.eq_true_equiv? := by
  intros t₁ t₂ heq rty lctx wf
  have ⟨heq, hequiv⟩ := LamEquiv.eq_true_equiv? _ wf heq
  cases heq; apply hequiv
...
theorem LamTerm.maxEVarSucc_eq_false_equiv?
  ...
  | .app _ (.app _ (.base (.eq _)) lhs) (.base .falseE), Eq.refl _ => by
    simp [mkNot, maxEVarSucc, Nat.max]
...
theorem LamGenConv.eq_false_equiv? : LamGenConv R? lval LamTerm.eq_false_equiv? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.eq_false_equiv? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.ne_true_equiv_eq_false?
  ...
  | .app _ (.base .not) (.app _ (.app _ (.base (.eq _)) lhs) (.base .trueE)), Eq.refl _ =>
    ...
    | .ofApp _ (.ofBase .ofNot) (.ofApp _ (.ofApp _ (.ofBase (.ofEq _)) Hlhs) (.ofBase .ofTrueE)) => by
      apply And.intro rfl
      apply LamEquiv.trans _ (prop_ne_equiv_eq_not _ Hlhs (.ofBase .ofTrueE))
      apply LamEquiv.congrArg _ (.ofApp _ (.ofBase (.ofEq _)) Hlhs)
      apply LamEquiv.not_true_equiv_false

theorem LamGenConv.ne_true_equiv_eq_false? : LamGenConv R? lval LamTerm.ne_true_equiv_eq_false? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.ne_true_equiv_eq_false? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.ne_false_equiv_eq_true?
  ...
  | .app _ (.base .not) (.app _ (.app _ (.base (.eq _)) lhs) (.base .falseE)), Eq.refl _ =>
    ...
    | .ofApp _ (.ofBase .ofNot) (.ofApp _ (.ofApp _ (.ofBase (.ofEq _)) Hlhs) (.ofBase .ofFalseE)) => by
      apply And.intro rfl
      apply LamEquiv.trans _ (prop_ne_equiv_eq_not _ Hlhs (.ofBase .ofFalseE))
      apply LamEquiv.congrArg _ (.ofApp _ (.ofBase (.ofEq _)) Hlhs)
      apply LamEquiv.not_false_equiv_true

theorem LamGenConv.ne_false_equiv_eq_true? : LamGenConv R? lval LamTerm.ne_false_equiv_eq_true? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.ne_false_equiv_eq_true? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.not_eq_true_equiv_eq_false?
  ...
  | .app _ (.app _ (.base (.eq _)) (.app _ (.base .not) lhs)) (.base .trueE), Eq.refl _ => by
    ...
    | .ofApp _ (.ofApp _ (.ofBase (.ofEq _)) (.ofApp _ (.ofBase .ofNot) Hlhs)) (.ofBase .ofTrueE) =>
      apply And.intro rfl
      apply trans _ (eqSymm? _ wft rfl)
      apply trans _ (symm _ (prop_ne_equiv_eq_not _ (.ofBase .ofTrueE) Hlhs))
      apply trans _ _ (ne_true_equiv_eq_false? _ (LamWF.mkNot (.mkEq Hlhs (.ofBase .ofTrueE))) rfl).right
      apply congrArg _ (.ofBase .ofNot); apply eqSymm? _ (.mkEq (.ofBase .ofTrueE) Hlhs) rfl

theorem LamGenConv.not_eq_true_equiv_eq_false? : LamGenConv R? lval LamTerm.not_eq_true_equiv_eq_false? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.not_eq_true_equiv_eq_false? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.not_eq_false_equiv_eq_true?
  ...
  | .app _ (.app _ (.base (.eq _)) (.app _ (.base .not) lhs)) (.base .falseE), Eq.refl _ => by
    ...
    | .ofApp _ (.ofApp _ (.ofBase (.ofEq _)) (.ofApp _ (.ofBase .ofNot) Hlhs)) (.ofBase .ofFalseE) =>
      apply And.intro rfl
      apply trans _ (eqSymm? _ wft rfl)
      apply trans _ (symm _ (prop_ne_equiv_eq_not _ (.ofBase .ofFalseE) Hlhs))
      apply trans _ _ (ne_false_equiv_eq_true? _ (LamWF.mkNot (.mkEq Hlhs (.ofBase .ofFalseE))) rfl).right
      apply congrArg _ (.ofBase .ofNot); apply eqSymm? _ (.mkEq (.ofBase .ofFalseE) Hlhs) rfl

theorem LamGenConv.not_eq_false_equiv_eq_true? : LamGenConv R? lval LamTerm.not_eq_false_equiv_eq_true? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.not_eq_false_equiv_eq_true? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.not_not_equiv?
  ...
  | .app _ (.base .not) (.app _ (.base .not) t'), Eq.refl _ =>
    ...
    apply And.intro rfl (LamEquiv.not_not_equiv _ wft.getArg.getArg)

theorem LamGenConv.not_not_equiv? : LamGenConv R? lval LamTerm.not_not_equiv? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.not_not_equiv? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.not_eq_equiv_eq_not?
  ...
  | .app _ (.app _ (.base (.eq _)) (.app _ (.base .not) lhs)) rhs, Eq.refl _ => by
    ...
    | .ofApp _ (.ofApp _ (.ofBase (.ofEq _)) (.ofApp _ (.ofBase .ofNot) Hlhs)) Hrhs =>
      apply And.intro rfl
      apply trans _ (eqSymm? _ (.mkEq (.mkNot Hlhs) Hrhs) rfl) (trans _ _ (prop_ne_equiv_eq_not _ Hlhs Hrhs))
      apply trans _ (symm _ (prop_ne_equiv_eq_not _ Hrhs Hlhs)) (neSymm? _ (.mkNot (.mkEq Hrhs Hlhs)) rfl)

theorem LamGenConv.not_eq_equiv_eq_not? : LamGenConv R? lval LamTerm.not_eq_equiv_eq_not? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.not_eq_equiv_eq_not? _ wf heq
  cases heq; apply hequiv
...
theorem LamEquiv.not_eq_not_equiv_eq?
  ...
  | .app _ (.app _ (.base (.eq _)) (.app _ (.base .not) lhs)) (.app _ (.base .not) rhs), Eq.refl _ => by
    ...
    | .ofApp _ (.ofApp _ (.ofBase (.ofEq _)) (.ofApp _ (.ofBase .ofNot) Hlhs)) (.ofApp _ (.ofBase .ofNot) Hrhs) =>
      apply And.intro rfl
      apply trans _ (symm _ (not_eq_equiv_eq_not? _ (.mkEq (.mkNot (.mkNot Hlhs)) Hrhs) rfl).right)
      apply congrFun _ (congrArg _ (.ofBase (.ofEq _)) (not_not_equiv _ Hlhs)) Hrhs

theorem LamGenConv.not_eq_not_equiv_eq? : LamGenConv R? lval LamTerm.not_eq_not_equiv_eq? := by
  intros t₁ t₂ heq lctx rty wf
  have ⟨heq, hequiv⟩ := LamEquiv.not_eq_not_equiv_eq? _ wf heq
  cases heq; apply hequiv
...
theorem LamGenConv.propext? : LamGenConv R? lval LamTerm.propext? := by
  intros t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.propext? _ wf heq
...
theorem LamTerm.maxEVarSucc_not_and_equiv_not_or_not?
  ...
  | .app _ (.base .not) (.app _ (.app _ (.base .and) lhs) rhs), Eq.refl _ => by
    simp [mkNot, mkOr, maxEVarSucc, Nat.max]
...
theorem LamGenConv.not_and_equiv_not_or_not? : LamGenConv R? lval LamTerm.not_and_equiv_not_or_not? := by
  intro t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.not_and_equiv_not_or_not? _ wf heq
  have ⟨wf', _⟩ := hequiv; cases (LamWF.unique wf wf').left; exact hequiv
...
theorem LamTerm.maxEVarSucc_not_or_equiv_not_and_not?
  ...
  | .app _ (.base .not) (.app _ (.app _ (.base .or) lhs) rhs), Eq.refl _ => by
    simp [mkNot, mkAnd, maxEVarSucc, Nat.max]
...
theorem LamGenConv.not_or_equiv_not_and_not? : LamGenConv R? lval LamTerm.not_or_equiv_not_and_not? := by
  intro t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.not_or_equiv_not_and_not? _ wf heq
  have ⟨wf', _⟩ := hequiv; cases (LamWF.unique wf wf').left; exact hequiv
...
theorem LamTerm.maxEVarSucc_propeq?
  ...
  | .app (.base .prop) (.app _ (.base (.eq _)) lhs) rhs, Eq.refl _ => by
    simp [mkOr, mkNot, mkAnd, maxEVarSucc, Nat.max]
...
theorem LamGenConv.propeq? : LamGenConv R? lval LamTerm.propeq? := by
  intro t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.propeq? _ wf heq
  have ⟨wf', _⟩ := hequiv; cases (LamWF.unique wf wf').left; exact hequiv
...
theorem LamGenConv.imp_equiv_not_or? : LamGenConv R? lval LamTerm.imp_equiv_not_or? := by
  intro t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.imp_equiv_not_or? _ wf heq
  have ⟨wf', _⟩ := hequiv; cases (LamWF.unique wf wf').left; exact hequiv
...
theorem LamGenConv.not_imp_equiv_and_not? : LamGenConv R? lval LamTerm.not_imp_equiv_and_not? := by
  intro t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.not_imp_equiv_and_not? _ wf heq
  have ⟨wf', _⟩ := hequiv; cases (LamWF.unique wf wf').left; exact hequiv
...
theorem LamTerm.maxEVarSucc_propne?
  ...
  | .app _ (.base .not) (.app (.base .prop) (.app _ (.base (.eq _)) lhs) rhs), Eq.refl _ => by
    simp [mkNot, mkAnd, mkOr, maxEVarSucc, Nat.max]
...
theorem LamGenConv.propne? : LamGenConv R? lval LamTerm.propne? := by
  intro t₁ t₂ heq lctx rty wf
  have hequiv := LamEquiv.propne? _ wf heq
  have ⟨wf', _⟩ := hequiv; cases (LamWF.unique wf wf').left; exact hequiv
